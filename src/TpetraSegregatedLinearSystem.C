...
int
TpetraSegregatedLinearSystem::getDofStatus(stk::mesh::Entity node)
{
...
TpetraSegregatedLinearSystem::beginLinearSystemConstruction()
{
  ...
  kokkos_parallel_for(
    "Nalu::TpetraSegregatedLinearSystem::beginLinearSystemConstructionA",
  ...
  for (unsigned inode = 0; inode < shared_not_owned_nodes.size(); ++inode) {
    ...
    sharedNotOwnedGids.push_back(static_cast<GlobalOrdinal>(naluId));
  ...
  ownedRowsMap_ = Teuchos::rcp(new LinSys::Map(
    Teuchos::OrdinalTraits<Tpetra::global_size_t>::invalid(), ownedGids, 1,
    tpetraComm));
...
int
TpetraSegregatedLinearSystem::insert_connection(
  stk::mesh::Entity a, stk::mesh::Entity b)
{
...
void
TpetraSegregatedLinearSystem::addConnections(
...
void
TpetraSegregatedLinearSystem::buildConnectedNodeGraph(
...
void
TpetraSegregatedLinearSystem::buildEdgeToNodeGraph(
...
void
TpetraSegregatedLinearSystem::buildFaceToNodeGraph(
  const stk::mesh::PartVector& parts)
{
  beginLinearSystemConstruction();
  stk::mesh::MetaData& metaData = realm_.meta_data();
...
void
TpetraSegregatedLinearSystem::buildElemToNodeGraph(
  const stk::mesh::PartVector& parts)
{
  beginLinearSystemConstruction();
...
void
TpetraSegregatedLinearSystem::buildReducedElemToNodeGraph(
  const stk::mesh::PartVector& parts)
{
  beginLinearSystemConstruction();
  stk::mesh::MetaData& metaData = realm_.meta_data();
...
void
TpetraSegregatedLinearSystem::buildFaceElemToNodeGraph(
  const stk::mesh::PartVector& parts)
{
  beginLinearSystemConstruction();
  stk::mesh::BulkData& bulkData = realm_.bulk_data();
...
void
TpetraSegregatedLinearSystem::buildNonConformalNodeGraph(
  const stk::mesh::PartVector& /* parts */)
{
  stk::mesh::BulkData& bulkData = realm_.bulk_data();
  beginLinearSystemConstruction();
  // if (realm_.bulk_data().parallel_rank()==0)
  // std::cerr<<"buildNonConformalNodeGraph"<<std::endl;
...
void
TpetraSegregatedLinearSystem::buildOversetNodeGraph(
  const stk::mesh::PartVector& /* parts */)
{
  // extract the rank
...
void
TpetraSegregatedLinearSystem::compute_send_lengths(
...
void
TpetraSegregatedLinearSystem::compute_graph_row_lengths(
  const std::vector<stk::mesh::Entity>& rowEntities,
  const std::vector<std::vector<stk::mesh::Entity>>& connections,
  ...
  stk::CommNeighbors& commNeighbors)
{
  ...
  for (size_t i = 0; i < rowEntities.size(); ++i) {
    ...
    add_to_length(
      hostLocallyOwnedRowLengths, hostSharedNotOwnedRowLengths, 1, lid_a,
      maxOwnedRowId_, entity_a_owned, numColEntities);

    const bool entity_a_shared = entity_a_status & DS_SharedNotOwnedDOF;
    if (entity_a_shared) {
      add_lengths_to_comm(
        bulk, commNeighbors, entity_a_owner, entityId_a, 1, numColEntities,
        colEntityIds.data(), colOwners.data());
    }
    ...
    for (size_t ii = 0; ii < numColEntities; ++ii) {
      ...
      add_to_length(
        hostLocallyOwnedRowLengths, hostSharedNotOwnedRowLengths, 1, lid_b,
        maxOwnedRowId_, entity_b_owned, 1);

      const bool entity_b_shared = entity_b_status & DS_SharedNotOwnedDOF;
      if (entity_b_shared) {
        add_lengths_to_comm(
          bulk, commNeighbors, colOwners[ii], entityId_b, 1, 1, &entityId_a,
          &entity_a_owner);
      }
    }
  }
...
void
TpetraSegregatedLinearSystem::insert_graph_connections(
  const std::vector<stk::mesh::Entity>& rowEntities,
  const std::vector<std::vector<stk::mesh::Entity>>& connections,
  ...
  for (size_t i = 0; i < rowEntities.size(); ++i) {
...

    {
      ...
      insert_single_dof_row_into_graph(
        crsGraph, entityToLIDHost_[entity_a.local_offset()], maxOwnedRowId_, 1,
        numColEntities, localDofs_b);
    }
    ...
    for (unsigned j = 0; j < numColEntities; ++j) {
      if (entities_b[j] != entity_a) {
        ...
        insert_single_dof_row_into_graph(
          ...
          maxOwnedRowId_, 1, 1, localDofs_a);
      }
    }
  }
}
...
TpetraSegregatedLinearSystem::fill_entity_to_col_LID_mapping()
{
  ...
  for (const stk::mesh::Bucket* bptr : nodeBuckets) {
    ...
    for (size_t i = 0; i < b.size(); ++i) {
      stk::mesh::Entity node = b[i];
      GlobalOrdinal gid = static_cast<GlobalOrdinal>(nodeIds[i]);
  ...
  for (const stk::mesh::Bucket* bptr : buckets) {
    ...
    for (stk::mesh::Entity node : bkt) {
      ...
      if (status & DS_SharedNotOwnedDOF) {
        ...
        GlobalOrdinal gid = static_cast<GlobalOrdinal>(naluId);
...
TpetraSegregatedLinearSystem::finalizeLinearSystem()
{
  ...
  communicate_remote_columns(
    bulkData, neighborProcs, commNeighbors, 1, ownedRowsMap_, ownedRowLengths,
    ownersAndGids_);
  ...
  insert_communicated_col_indices(
    neighborProcs, commNeighbors, 1, ownedGraph, *ownedRowsMap_,
    *totalColsMap_);
  ...
  sharedNotOwnedRhs_ =
    Teuchos::rcp(new LinSys::MultiVector(sharedNotOwnedRowsMap_, numDof_));
...
KOKKOS_FUNCTION
void
TpetraSegregatedLinearSystem::TpetraLinSysCoeffApplier::resetRows(
  unsigned numNodes,
...
KOKKOS_FUNCTION
void
TpetraSegregatedLinearSystem::TpetraLinSysCoeffApplier::operator()(
  unsigned numEntities,
  ...
  const char* /* trace_tag */)
{
  ...
  for (size_t i = 0; i < n_obj; i++) {
    scratchIds[i] = entityToColLIDHost_[entities[i].local_offset()];
  ...
  for (unsigned r = 0; r < numRows; r++) {
    ...
    const double* const cur_lhs =
      &lhs[(cur_perm_index * numDof_) * (numRows * numDof_)];
  ...
  const unsigned endPos)
{
  ...
  for (const stk::mesh::Bucket* bptr : buckets) {
    ...
    const double* solution =
      (double*)stk::mesh::field_data(*solutionField, *b.begin());
    const double* bcValues =
      (double*)stk::mesh::field_data(*bcValuesField, *b.begin());
    ...
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      ...
      const stk::mesh::EntityId naluId =
        *stk::mesh::field_data(*realm_.naluGlobalId_, entity);
      const LocalOrdinal localIdOffset =
        lookup_myLID(myLIDs_, naluId, "applyDirichletBCs");

      const bool useOwned = localIdOffset < maxOwnedRowId_;
      const LocalOrdinal actualLocalId =
        useOwned ? localIdOffset : localIdOffset - maxOwnedRowId_;
      Teuchos::RCP<LinSys::Matrix> matrix =
        useOwned ? ownedMatrix_ : sharedNotOwnedMatrix_;
      const LinSys::LocalMatrix& local_matrix =
        useOwned ? getOwnedLocalMatrix() : getSharedNotOwnedLocalMatrix();
      ...
      if (localIdOffset > maxSharedNotOwnedRowId_) {
        std::cerr << "localId > maxSharedNotOwnedRowId_:: localId= "
                  ...
                  << " useOwned = " << (localIdOffset < maxOwnedRowId_)
                  << std::endl;
        throw std::runtime_error(
          "logic error: localId > maxSharedNotOwnedRowId_");
      }
      ...
      if (rowLength > 0) {
        ...
        local_matrix.replaceValues(
          actualLocalId, &indices[0], rowLength, new_values.data(),
          internalMatrixIsSorted);
      }
...
int
TpetraSegregatedLinearSystem::solve(stk::mesh::FieldBase* linearSolutionField)
{
  ...
  if (realm_.get_activate_memory_diagnostic()) {
    NaluEnv::self().naluOutputP0()
      << "NaluMemory::TpetraSegregatedLinearSystem::solve() PreSolve: "
      << eqSysName_ << std::endl;
...
bool
TpetraSegregatedLinearSystem::checkForZeroRow(
  bool useOwned, bool doThrow, bool doPrint)
{
  Teuchos::RCP<LinSys::Matrix> matrix =
    useOwned ? ownedMatrix_ : sharedNotOwnedMatrix_;
  Teuchos::RCP<LinSys::MultiVector> rhs =
  ...
  kokkos_parallel_for(
    "Nalu::TpetraSegregatedLinearSystem::checkForZeroRowA", n,
    [&](const size_t& i) {
  ...
  kokkos_parallel_for(
    "Nalu::TpetraSegregatedLinearSystem::checkForZeroRowC", nrowG,
    [&](const size_t& ii) {
      ...
      if (
        ...
        if (doPrint) {
          ...
          std::cout << "P[" << bulkData.parallel_rank() << "] LHS zero: " << ii
                    << " GID= " << gid << " nid= " << nid << " naluGlobalId "
                    << naluGlobalId << " is_valid= " << bulkData.is_valid(node)
...
void
TpetraSegregatedLinearSystem::writeToFile(
...
void
TpetraSegregatedLinearSystem::writeSolutionToFile(
  const char* base_filename, bool useOwned)
{
  stk::mesh::BulkData& bulkData = realm_.bulk_data();
  const unsigned p_rank = bulkData.parallel_rank();
  ...
  stk::mesh::FieldBase* stkField)
{
  ...
  for (size_t ib = 0; ib < buckets.size(); ++ib) {
    ...
    const stk::mesh::EntityId* naluGlobalId =
      stk::mesh::field_data(*realm_.naluGlobalId_, *b.begin());
    for (stk::mesh::Bucket::size_type k = 0; k < length; ++k) {
      stk::mesh::Entity node = b[k];
      const LocalOrdinal localIdOffset = entityToLIDHost_[node.local_offset()];
      for (unsigned dofIdx = 0; dofIdx < fieldSize; ++dofIdx) {
        ...
        if (!useOwned) {
          ...
          std::cout << "P[" << p_rank << "] useOwned = " << useOwned
                    << " localId = " << localId
                    << " maxOwnedRowId_= " << maxOwnedRowId_
                    << " actualLocalId= " << actualLocalId
                    << " naluGlobalId= " << naluGlobalId[k]
