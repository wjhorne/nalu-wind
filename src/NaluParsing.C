...
void
operator>>(const YAML::Node& node, WallBoundaryConditionData& wallBC)
{
...
void
operator>>(const YAML::Node& node, InflowBoundaryConditionData& inflowBC)
{
...
void
operator>>(const YAML::Node& node, OpenBoundaryConditionData& openBC)
{
...
void
operator>>(const YAML::Node& node, OversetBoundaryConditionData& oversetBC)
{
...
// helper function for symmetry and abltop BC
SymmetryUserData
get_symmetry_data(const YAML::Node& node)
{
  const YAML::Node& symmetryUserData = node["symmetry_user_data"];
  auto data = symmetryUserData.as<SymmetryUserData>();
  if (symmetryUserData["symmetry_type"]) {
    ...
    if (symmType == "generalized_weak" || symmType == "default") {
      ...
      data.symmType_ = SymmetryUserData::SymmetryTypes::X_DIR_STRONG;
    } else if (symmType == "y_direction_strong") {
      data.symmType_ = SymmetryUserData::SymmetryTypes::Y_DIR_STRONG;
    } else if (symmType == "z_direction_strong") {
      data.symmType_ = SymmetryUserData::SymmetryTypes::Z_DIR_STRONG;
    } else {
      throw std::runtime_error(
...
void
operator>>(const YAML::Node& node, SymmetryBoundaryConditionData& symmetryBC)
{
  ...
  symmetryBC.userData_ = get_symmetry_data(node);
  const YAML::Node& symmetryUserData = node["symmetry_user_data"];
  if (symmetryUserData["use_projections"]) {
    symmetryBC.userData_.useProjections_ =
      symmetryUserData["use_projections"].as<bool>();
  }
}

void
operator>>(const YAML::Node& node, ABLTopBoundaryConditionData& abltopBC)
{
  abltopBC.bcName_ = node["abltop_boundary_condition"].as<std::string>();
  ...
  if (node["symmetry_user_data"]) {
    abltopBC.symmetryUserData_ = get_symmetry_data(node);
  }
}

void
operator>>(const YAML::Node& node, PeriodicBoundaryConditionData& periodicBC)
{
  periodicBC.bcName_ = node["periodic_boundary_condition"].as<std::string>();
  periodicBC.masterSlave_ = node["target_name"].as<MasterSlave>();
  periodicBC.targetName_ =
    periodicBC.masterSlave_.master_ + "_" + periodicBC.masterSlave_.slave_;
...
void
operator>>(
  const YAML::Node& node, NonConformalBoundaryConditionData& nonConformalBC)
{
  nonConformalBC.bcName_ =
    node["non_conformal_boundary_condition"].as<std::string>();
  ...
  // check for old/new syntax; one of the two is required to define the
  // current/opposing part(s)
  ...
  if (targetPairName) {
    ...
    nonConformalBC.opposingPartNameVec_[0] =
    ...
    if (targetsCurrent.Type() == YAML::NodeType::Scalar) {
      ...
      for (size_t i = 0; i < targetsCurrent.size(); ++i) {
        nonConformalBC.currentPartNameVec_[i] =
          targetsCurrent[i].as<std::string>();
      }
    }
...
operator>>(const YAML::Node& node, UserFunctionInitialConditionData& fcnIC)
{
  ...
  if (expect_map(node, "user_function_name", false)) {
    fcnIC.functionNames_ =
      node["user_function_name"].as<std::map<std::string, std::string>>();
  }

  if (expect_map(node, "user_function_parameters", true)) {
    fcnIC.functionParams_ = node["user_function_parameters"]
...
bool
case_insensitive_compare(std::string s1, std::string s2)
{
  ...
  const Node& node, sierra::nalu::VolumeOfFluid& vof)
{
  ...
  vof.volumeOfFluid_ = node.as<double>();
  ...
  const Node& node, sierra::nalu::WallUserData& wallData)
{
  ...
  // If the wall is treated as an atmospheric boundary layer surface stress
  // model.
  ...
  const Node& node, sierra::nalu::OpenUserData& openData)
{
  ...
  if (node["total_pressure"]) {
    openData.totalP_ = node["total_pressure"].as<bool>();
  }
  ...
  const Node& node, sierra::nalu::SymmetryUserData& /* symmetryData */)
{
  ...
  if (node["normal_temperature_gradient"])
    throw std::runtime_error(
      "SymmetryBoundaryConditionData: Normal temperature gradient for ABL top "
      "boundary must use abltop_boundary_condition");
  return true;
}
  ...
  const Node& node, sierra::nalu::NonConformalUserData& nonConformalData)
{
  ...
  if (node["expand_box_percentage"]) {
    nonConformalData.expandBoxPercentage_ =
      node["expand_box_percentage"].as<double>();
  }
  if (node["clip_isoparametric_coordinates"]) {
...
bool
convert<std::map<std::string, std::vector<std::string>>>::decode(
  const YAML::Node& node,
  std::map<std::string, std::vector<std::string>>& mapName)
{
